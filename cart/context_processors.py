...
def cart_item(request):
    ...
    if request.user.is_authenticated:
        ...
        summed_price = sum(item.quantity * item.product.price for item in cart_item)
        formatted_sub_total_price = format_currency(summed_price, 'EUR', locale='en_US')
        ...
        for item in carts:
            if isinstance(item, dict) and 'product_id' in item and 'quantity' in item:
                product = get_object_or_404(Product, pk=item['product_id'])
                cart_item.append({'product': product, 'quantity': item['quantity']})
                summed_price += Decimal(item['quantity']) * Decimal(product.price)
                formatted_sub_total_price = format_currency(summed_price, 'EUR', locale='en_US')
